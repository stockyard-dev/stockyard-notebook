...
func TestTrialMiddleware_writesBlocked(t *testing.T) {
	...
	cases := []struct {
		...
		{"POST", "/api/notebooks"},
		{"PUT", "/api/notebooks/123"},
		{"DELETE", "/api/notebooks/123"},
		{"PUT", "/api/extras/notebooks/123"},
...
func TestTrialMiddleware_proTierBypassesGate(t *testing.T) {
	...
	req := httptest.NewRequest("POST", "/api/notebooks", bytes.NewReader(body))
	w := httptest.NewRecorder()
	srv.ServeHTTP(w, req)
	if w.Code == http.StatusPaymentRequired {
...
// newTestDB is a local helper since notebook has no existing test file
// that defines one.
func newTestDB(t *testing.T) *store.DB {
	t.Helper()
	dir, err := os.MkdirTemp("", "notebook-trial-test-*")
	if err != nil {
